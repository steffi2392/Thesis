...
public class DoubleGraph {
		...
		while (true) {
			...
			if (!visited.contains(currEndpoint.getId())) {
				...
				if (currEndpoint.isOdd() && (currPath.size() > 1 
						&& currPath.get(currPath.size() - 2).getId() != 
						(currEndpoint.hasOtherEnd() ? currEndpoint.getOtherEnd().getId() : currEndpoint.getId()))) {
	...
	public void connectDisconnectedOdds(Bipartite otherEdges) {
		...
		for (Endpoint disconnect : disconnected) {
			System.out.println("In connectDisconnectedOdds. disconnect = " + disconnect.getChain().getCharId()); 
			if (disconnect.isDisconnected()) { // make sure it's still disconnected!
				...
				for (Vertex adj : disconnectedStart.getAdjList()) {
					...
					System.out.println("breaking chain " + endpoint.getChain().getCharId() + " at connection (" 
							+ disconnectedStart.getId() + ", " + adj.getId() + ")");
					int index = findIndexWithinChain(adj.getId(), endpoint.getChain().getVertexList());	
	...
	private List<Endpoint> getDisconnected() {
		...
		for (Endpoint e : graph.values()) {
			if (!lookedAt.contains(e.getChain().getId())) {
				if (e.isOdd() && e.getAdjList().isEmpty() && (!e.hasOtherEnd() || e.getOtherEnd().getAdjList().isEmpty())) {
					disconnected.add(e);
				}
			}
		}
