...
public class Driver3 {
	public static void main(String[] args) {
		...
		//System.out.printf("%-10s %-10s %-10s %-10s %-10s\n", "Count", "N", "d", "No Go Home", "Go Home");
		System.out.printf("%-10s %-10s %-10s %-10s\n", "Count", "N", "d", "Go Home");
		
		int runNumber = 0; 
		for (int N = 10; N < 200000; N = N * 2) {
			for (int d = 3; d < 17; d += 2) {
				for (int i = 0; i < 100; i++) { 
					...
					//	numTraversedList, edgesPerDummy, 0, N, false /* go home */, d);
					/*if (cycles == null) {
					...
					ArrayList<Deque<Edge>> cycles = bipartiteGraphCopy.findAllCycles(numLookedAtList, 
						numTraversedList, edgesPerDummy, 0, N, true /* go home */, d);
					if (cycles == null) {
						System.out.println("Didn't find cycle");
						return; 
					}
